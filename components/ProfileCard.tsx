...
const StatItem: React.FC<{ value: number; label: string }> = ({ value, label }) => (
  <div className="text-center">
    <p className="text-lg sm:text-xl font-bold text-white">{new Intl.NumberFormat().format(value)}</p>
    <p className="text-xs sm:text-sm text-gray-400">{label}</p>
  </div>
);

const VerifiedIcon: React.FC = () => (
    <svg viewBox="0 0 22 22" className="h-4 w-4 sm:h-5 sm:w-5 fill-current text-blue-500" aria-label="Verified account">
...
const ProfileCard: React.FC<ProfileCardProps> = ({ data }) => {
  return (
    <div className="mt-10 w-full max-w-2xl mx-auto bg-gray-900/50 backdrop-blur-sm border border-gray-700 rounded-3xl shadow-lg shadow-purple-500/10 p-6 sm:p-8 transition-all duration-300 animate-fade-in">
      <div className="flex flex-col sm:flex-row items-center sm:items-start space-y-4 sm:space-y-0 sm:space-x-8">
        <img
          src={data.profilePicUrl}
          alt={`${data.username}'s profile picture`}
          className="w-20 h-20 sm:w-24 sm:h-24 rounded-full border-4 border-gray-700 object-cover shadow-md"
        />
        <div className="flex-1 text-center sm:text-left">
          <div className="flex items-center justify-center sm:justify-start gap-2 mb-1 sm:mb-2">
            <h1 className="text-xl sm:text-2xl font-bold text-white">{data.username}</h1>
            {data.isVerified && <VerifiedIcon />}
          </div>
          <h2 className="text-base sm:text-lg text-gray-300">{data.fullName}</h2>
        </div>
      </div>

      <div className="mt-6 sm:mt-8 grid grid-cols-3 gap-4 border-t border-b border-gray-700 py-3 sm:py-4">
      ...
      <div className="mt-4 sm:mt-6">
        <p className="text-sm text-gray-300 whitespace-pre-wrap">{data.biography}</p>
      </div>

      {data.isPrivate && (
        <div className="mt-4 sm:mt-6 text-center text-yellow-400 bg-yellow-900/50 border border-yellow-700 rounded-lg p-2 sm:p-3">
