...
const TypingText: React.FC<TypingTextProps> = ({ text, speed = 70, className }) => {
  ...
  // Pré-processa o texto em segmentos, identificando palavras com gradiente
  const textSegments = useMemo(() => {
    const segments: { content: string; isGradient: boolean }[] = [];
    // Divide o texto, mantendo 'INSTAGRAM' e 'SEM SENHA' como itens separados no array
    const parts = text.split(/(INSTAGRAM|SEM SENHA)/g); 
    
    parts.forEach(part => {
      if (part) { // Garante que partes vazias não sejam adicionadas
        segments.push({ content: part, isGradient: part === 'INSTAGRAM' || part === 'SEM SENHA' });
      }
    });
    return segments;
  }, [text]);
