...
const WebSuggestions: React.FC<WebSuggestionsProps> = ({ profileData }) => {
  return (
    <aside className="hidden lg:block w-80 flex-shrink-0 p-8">
      <div className="flex items-center gap-4 mb-8">
        ...
        <button className="ml-auto text-blue-400 text-xs font-semibold">Mudar</button>
      </div>

      <div className="flex justify-between items-center mb-4">
        <p className="text-gray-400 font-semibold text-sm">Sugestões para você</p>
        <a href="#" className="text-white text-xs font-semibold">Ver tudo</a>
      </div>
      ...
      <div className="space-y-4">
        {mockSuggestions.map(suggestion => (
          <div key={suggestion.username} className="flex items-center gap-3">
            ...
            <button className="ml-auto text-blue-400 text-xs font-semibold">Seguir</button>
          </div>
        ))}
      </div>
